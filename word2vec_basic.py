...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  #Making sure there are enough elements on the left of the center word at the begining
  if data_index<skip_window:
    data_index = skip_window

  #Below code generates batches by taking one word from each side of the centre word, num_skips times
  for i in range(batch_size//(num_skips)):
    for j in range(num_skips):
      if j % 2 == 0:
        batch[(i* num_skips)+j] = data[data_index]
        labels[(i * num_skips) + j] = data[data_index + (j//2)+1]
      if j % 2 != 0:
        batch[(i * num_skips) + j] = data[data_index]
        labels[(i * num_skips) + j] = data[data_index - (j // 2) - 1]
